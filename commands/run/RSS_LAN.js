...
module.exports.run = async (bot) => {
	/*
setTimeout(() =>{
setInterval(RSSFULL_LAN,12000);
},3000);
	*/
function RSSFULL_LAN(){
fetch(RSS.Feed.Lan).then(res => res.text()).then(body => 
			...
			for (var i = 0; i < 15; i++){
			 if(!old_info[0].includes(new_info[0][i].toLowerCase().replace(/[!¡ &\/\\#,_+()$~%.'":*?<>{}-]/g, ""))){
				...
				bot.channels.get(`<ID>`).send({embed: {
					...
					image:{
							url:`https://lan.leagueoflegends.com${new_info[3][i].substring(new_info[3][i].indexOf('/sites'),new_info[3][i].indexOf('?itok=')).replace(/large/g, "scale_xlarge")}`
							}
					  }
				});
