...
var id_delay_LAS,id_delay_all_LAS,messsage_delay_LAS,server_status_delay_LAS;
...
RSS(servers.status.Las).then(info => resolv(info))
},10*1000)
...
if(!info)return;
const rol = bot.guilds.get('<ID>').roles.find( rol=> rol.name === 'Notificación');
...
function Update(){
    setTimeout(()=>{
        id_delay_all_LAS = id_all;
        id_delay = id;
        messsage_delay_LAS = messsage;
        server_status_delay_LAS = server_status;
    },5000);
};
...
for (var y = 0;y< id.length; y++){
    for (var x = 0; x< id[y].length; x++){
        for (var z = 0; z< id[y][x].length ;z++){
                if(id_delay_all_LAS.includes(id[y][x][z])){
                }else{
                        Update();
    ...
    for (var x = 0; x< id_delay_LAS[y].length; x++){
        for (var z = 0; z< id_delay_LAS[y][x].length ;z++){
                if(id_all.includes(id_delay_LAS[y][x][z])){
                ...
                alert.send({embed:{
                    ...
                    fields:[{
                        name:'Tipo',
                        value:` ${status[messsage_delay_LAS[y][x][z][0]]}`
                    },{
                        name:'Contenido',
                        value:`${messsage_delay_LAS[y][x][z][1]}`
                    }]
...
if(server_status[0] !== server_status_delay_LAS[0]){
